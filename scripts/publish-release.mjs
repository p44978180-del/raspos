...
const TAG = process.env.RELEASE_TAG || (process.argv.find((a) => a.startsWith("--tag=")) ? process.argv.find((a) => a.startsWith("--tag=")).split("=")[1] : "v3.1.0")
const RELEASE_NAME = `🌾 РГАУ Расписание ${TAG} — Масштабный редизайн: реальные GPS-данные кампуса, плавные анимации, чистый минималистичный UI`

const RELEASE_BODY = `## 🌾 РГАУ Расписание ${TAG} — Масштабный редизайн и актуализация данных кампуса

Официальное кроссплатформенное приложение расписания РГАУ-МСХА имени К.А. Тимирязева — Enterprise SuperApp v3.1.0 (на базе Next-Gen стека v3.0.1), масштабный редизайн и актуализация всех данных о кампусе.

---

### ✨ Ключевые изменения в релизе v3.1.0:

1. **🗺️ Реальные GPS-координаты всех корпусов кампуса**:
   - Исследованы и верифицированы точные координаты 19 зданий РГАУ-МСХА через официальные источники (timacad.ru, Яндекс.Карты).
   - Маркеры на карте теперь точно соответствуют реальному расположению: УК-1 (Тимирязевская, 49), УК-2, УК-3, УК-4, УК-6 (Зоотехния), УК-8, УК-9, УК-10, УК-12, К-17, УК-26, Инж-28, Цифровой центр-29, Агрохим, ЦНБ, Ректорат, Столовая-КП, СОК.
   - 13 общежитий с точными адресами: №1–9, 11, 13, 16.

2. **🎬 Полный рерайт системы анимаций**:
   - Устранена проблема «наложения экранов» при переходе между вкладками.
   - Новая система: GPU-accelerated \`opacity + translateY + scale\` fade с \`contain: layout style paint\` для полной изоляции содержимого вкладки.
   - \`will-change: opacity, transform\` + \`backface-visibility: hidden\` — аппаратное ускорение на всех устройствах.
   - Длительность перехода: 210ms \`cubic-bezier(0.16, 1, 0.3, 1)\` — плавный, без рывков.

3. **📱 Исправление черной полосы сверху (iOS)**:
   - Изменен \`apple-mobile-web-app-status-bar-style: black-translucent\` в index.html — Safari iOS отображает контент под полосой состояния.
   - StatusBar div теперь использует точный \`height: env(safe-area-inset-top, 0px)\` через inline style с \`background: var(--color-bg)\`.
   - Результат: полное совпадение цвета шапки приложения и системной полосы состояния в светлой и тёмной темах.

4. **🎨 Очистка дизайна (борьба с «нейрослопом»)**:
   - Консистентная цветовая схема: primary зелёный РГАУ #2D5016 / #15803D, нейтральные серые, белый/тёмный фон.
   - Минималистичный дизайн карточек: большие отступы, мягкие тени.
   - Bottom Nav: чистые иконки + label без лишних элементов.

5. **📋 Официальное расписание звонков (актуализировано)**:
   - 1-я пара: 08:30–10:05 | 2-я: 10:20–11:55 | 3-я: 12:25–14:00
   - 4-я: 14:15–15:50 | 5-я: 16:05–17:40 | 6-я: 17:55–19:30 | 7-я: 19:45–21:20
   - Обеденный перерыв 30 мин после 2-й пары (11:55–12:25), остальные — 15 мин.
...
---
*РГАУ-МСХА имени К.А. Тимирязева · Основан в 1865 году · v3.1.0 Design Overhaul*
`
